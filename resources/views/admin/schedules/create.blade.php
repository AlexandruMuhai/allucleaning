...
@section('header', 'Dodaj dzień sprzątania')
...
<div class="mx-auto max-w-lg">
    <div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <form method="POST" action="{{ route('admin.schedules.store', $location) }}" class="space-y-5 p-6">
            @csrf
            <div>
                ...
                <select id="day_of_week" name="day_of_week" required class="w-full rounded-xl border-slate-300 px-4 py-3 text-sm focus:border-emerald-500 focus:ring-emerald-500">
                    @for ($d = 1; $d <= 6; $d++)
                        <option value="{{ $d }}" {{ old('day_of_week') == $d ? 'selected' : '' }}>{{ \Carbon\Carbon::createFromIsoFormat('N', $d)->translatedFormat('l') }}</option>
                    @endfor
                    <option value="0" {{ old('day_of_week') == 0 ? 'selected' : '' }}>Niedziela</option>
                </select>
            </div>
            <div>
                <label for="start_time" class="mb-2 block text-sm font-medium text-slate-700">Godzina rozpoczęcia</label>
                <input id="start_time" type="time" name="start_time" value="{{ old('start_time', '18:00') }}" required class="w-full rounded-xl border-slate-300 px-4 py-3 text-sm focus:border-emerald-500 focus:ring-emerald-500">
            </div>
            <div>
                ...
                <select id="default_employee_id" name="default_employee_id" class="w-full rounded-xl border-slate-300 px-4 py-3 text-sm focus:border-emerald-500 focus:ring-emerald-500">
                    ...
                    @foreach ($employees as $emp)
                        <option value="{{ $emp->id }}" {{ old('default_employee_id') == $emp->id ? 'selected' : '' }}>{{ $emp->name }}</option>
                    @endforeach
                </select>
            </div>
