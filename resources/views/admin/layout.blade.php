...
<head>
    ...
    <title>@yield('title', 'Panel administracyjny') — {{ config('app.name') }}</title>
    ...
    <div x-data="{ sidebarOpen: false }" class="min-h-full lg:flex">
        ...
        <aside class="fixed inset-y-0 left-0 z-40 w-64 -translate-x-full transform bg-slate-900 transition-transform duration-300 lg:static lg:translate-x-0"
            ...
            <nav class="flex-1 space-y-1 px-3 py-5">
                ...
                <a href="{{ route('admin.dashboard') }}"
                   class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors {{ request()->routeIs('admin.dashboard') ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}">
                ...
                <a href="{{ route('admin.schedule.index') }}"
                   class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors {{ request()->routeIs('admin.schedule.*') ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}">
                ...
                <a href="{{ route('admin.locations.index') }}"
                   class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors {{ request()->routeIs('admin.locations.*') ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}">
                    <svg class="size-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                    Lokalizacje
                </a>

                <a href="{{ route('admin.jobs.index') }}"
                   class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors {{ request()->routeIs('admin.jobs.*') ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}">
                ...
                <a href="{{ route('admin.passports.index') }}"
                   class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors {{ request()->routeIs('admin.passports.*') ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}">
                    <svg class="size-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"/></svg>
                    Paszporty QR
                </a>

                <a href="{{ route('admin.issue-reports.index') }}"
                   class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors {{ request()->routeIs('admin.issue-reports*') ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}">
                ...
                <a href="{{ route('admin.users.index') }}"
                   class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors {{ request()->routeIs('admin.users.*') ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}">
                ...
                <a href="{{ route('admin.clean-logs.create') }}"
                   class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors {{ request()->routeIs('admin.clean-logs.create') ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}">
        ...
        <div class="flex min-w-0 flex-1 flex-col">
            ...
            <header class="sticky top-0 z-20 flex h-16 items-center gap-4 border-b border-slate-200 bg-white/80 px-4 backdrop-blur-md sm:px-6 lg:px-8">
                ...
                <h1 class="font-display text-lg font-bold text-slate-900">@yield('header', 'Panel administracyjny')</h1>
                ...
                <div class="ml-auto flex items-center gap-3">
                    <div class="flex items-center gap-3 rounded-full bg-slate-50 px-3 py-1.5">
                        ...
                        <div class="hidden text-right sm:block">
                            <p class="text-sm font-semibold leading-tight text-slate-900">{{ auth()->user()->name }}</p>
                            <p class="text-xs leading-tight text-slate-500">{{ auth()->user()->roleLabel() }}</p>
                        </div>
                    </div>
                </div>
            </header>
