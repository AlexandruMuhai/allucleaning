...
@section('header', 'Lokalizacje')

@section('content')
<div class="space-y-6">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p class="text-sm text-slate-500">Łącznie: <span class="font-semibold text-slate-700">{{ $locations->total() }}</span></p>
        @if (auth()->user()->isAdministrator())
        <a href="{{ route('admin.locations.create') }}" class="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-emerald-600/25 transition-all hover:bg-emerald-500 active:scale-[0.98]">
        ...
        <table class="min-w-full divide-y divide-slate-200">
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($locations as $location)
                    <tr class="transition-colors hover:bg-slate-50 {{ !$location->is_active ? 'opacity-60' : '' }}">
                        <td class="px-6 py-4">
                            <a href="{{ route('admin.locations.show', $location) }}" class="text-sm font-semibold text-slate-900 hover:text-emerald-600">{{ $location->name }}</a>
                            @if ($location->address)<p class="text-xs text-slate-400 mt-0.5">{{ $location->address }}</p>@endif
                        </td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $location->type === 'office' ? 'bg-blue-50 text-blue-700 ring-1 ring-inset ring-blue-600/20' : 'bg-violet-50 text-violet-700 ring-1 ring-inset ring-violet-600/20' }}">
                                {{ $location->typeLabel() }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-sm text-slate-600">{{ $location->client?->name ?? '—' }}</td>
                        <td class="px-6 py-4 text-sm font-semibold text-slate-700">{{ $location->passports_count }}</td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.locations.show', $location) }}" class="rounded-lg p-2 text-slate-500 transition-colors hover:bg-emerald-50 hover:text-emerald-600" title="Szczegóły">
                                    <svg class="size-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                </a>
                                @if (auth()->user()->isAdministrator())
                                <a href="{{ route('admin.locations.edit', $location) }}" class="rounded-lg p-2 text-slate-500 transition-colors hover:bg-blue-50 hover:text-blue-600" title="Edytuj">
                                ...
                                <form method="POST" action="{{ route('admin.locations.destroy', $location) }}" onsubmit="return confirm('Usunąć obiekt?');">
                                    @csrf @method('DELETE')
