...
<head>
    ...
    <title>Paszport Czystości — <?php echo e($passport->name); ?></title>
...
<body class="min-h-full bg-slate-50 font-sans text-slate-800 antialiased">
    <div class="mx-auto flex min-h-full max-w-md flex-col px-4 py-6">
        ...
        <header class="mb-6 text-center">
            ...
            <h1 class="font-display text-2xl font-extrabold text-slate-900"><?php echo e($passport->name); ?></h1>
            <?php if($passport->location): ?>
                <p class="mt-1 text-sm text-slate-500"><?php echo e($passport->location->name); ?></p>
            <?php endif; ?>
        </header>
        ...
        <section class="mb-6 rounded-3xl bg-gradient-to-br from-emerald-500 to-teal-600 p-6 text-center text-white shadow-lg shadow-emerald-600/20">
            ...
            <?php if($lastClean): ?>
                ...
                <p class="mt-1 text-sm text-emerald-50">
                    Ostatnio sprzątane:
                    <span class="font-semibold"><?php echo e($lastClean->cleaned_at->translatedFormat('d.m.Y')); ?> o <?php echo e($lastClean->cleaned_at->format('H:i')); ?></span>
                </p>
                <?php if($lastClean->user): ?>
                    <p class="text-sm text-emerald-50">przez <?php echo e($lastClean->user->name); ?></p>
                <?php endif; ?>
            <?php else: ?>
            ...
            <?php if($passport->next_scheduled_clean): ?>
                <div class="mt-4 inline-block rounded-full bg-white/15 px-4 py-1.5 text-xs font-medium">
                    Planowane sprzątanie: <?php echo e($passport->next_scheduled_clean->translatedFormat('d.m.Y H:i')); ?>
        ...
        <section class="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" x-data="{ open: false }">
            ...
            <form method="POST" action="<?php echo e(route('passport.issue', $passport->uuid)); ?>" enctype="multipart/form-data" x-show="open" x-cloak class="mt-4 space-y-4">
                <?php echo csrf_field(); ?>
                <div>
        ...
        <?php if($passport->cleanLogs->isNotEmpty()): ?>
            <section class="mb-6">
                <h2 class="mb-3 px-1 font-display text-sm font-bold uppercase tracking-wider text-slate-500">Ostatnie sprzątania</h2>
                <div class="space-y-2">
                    <?php $__currentLoopData = $passport->cleanLogs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $log): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="flex items-center gap-3 rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
                            <?php if($log->photo_path): ?>
                                <img src="<?php echo e(asset('storage/' . $log->photo_path)); ?>" alt="Zdjęcie" class="size-12 rounded-lg object-cover">
                            <?php else: ?>
                            ...
                            <div class="min-w-0 flex-1">
                                <p class="text-sm font-semibold text-slate-900"><?php echo e($log->cleaned_at->translatedFormat('d.m.Y H:i')); ?></p>
                                <p class="truncate text-xs text-slate-500"><?php echo e($log->user->name ?? 'Pracownik'); ?><?php echo e($log->notes ? ' — ' . $log->notes : ''); ?></p>
                            </div>
                        </div>
...
<?php /**PATH /var/www/html/resources/views/public/passport.blade.php ENDPATH**/ ?>